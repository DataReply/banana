...
define([
  ...
  function(angular, app, _, kbn, moment) {
    ...
    var module = angular.module('kibana.panels.searchHighlight', []);
    app.useModule(module);
    module.controller('searchHighlight', function($rootScope, $scope, fields, querySrv, dashboard, filterSrv) {
        $scope.panelMeta = {
          modals: [{
            description: "Inspect",
            icon: "fa fa-info",
          ...
          editorTabs: [{
            title: 'Paging',
            src: 'app/panels/searchHighlight/pagination.html'
          }, {
            title: 'Queries',
      ...
      $scope.get_data = function(segment, query_id) {
        ...
        var highlight = '&hl='+($scope.panel.highlight.length > 0).toString();
    ...
    module.filter('tableHighlight', function() {
      return function(text) {
        if (!_.isUndefined(text) && !_.isNull(text) && text.toString().length > 0) {
          ...
          replace(/^\s*\n/gm, '<br>').
          replace(/<em>/g, '<span class="highlight"><b>').
          replace(/<\/em>/g, '</b></span>');
